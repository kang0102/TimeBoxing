...
(function(root,factory){if(typeof module==='object'&&module.exports)module.exports=factory(require('./rotation_decisions.js'));else root.PortfolioLogic=factory(root.RotationDecisions);})(typeof globalThis!=='undefined'?globalThis:this,function(decisions){
  ...
  const priceText=n=>n.toLocaleString('zh-TW',{maximumFractionDigits:2});
  ...
  function assess(p,row,data,now=Date.now()){
    ...
    const nextEntry=positive(nextHigh)&&positive(nextMeanVolume)?`${price.phase==='ready'?'本次收盤四項價量已達標。':''}下一交易日以 ${row.as_of} 日線準備：等收盤突破 ${priceText(nextHigh)}、全日成交量至少 ${Math.ceil(nextMeanVolume*1.5).toLocaleString('zh-TW')} 股（基準均量 1.5 倍），並守住趨勢線、近 5 日強於大盤；當日法人／資金資料也須補齊且非偏賣。盤中越過不算收盤確認，長期計畫另需核對基本面。`:'下一交易日的前高或均量基準未齊；先等更新，不沿用本次收盤的舊突破門檻。';
    const next=risk?`先處理自訂風險線 ${priceText(lossLine)}；不要把等反彈當作已驗證的退出策略。`:weak?`先等收盤重新站回 ${label} ${priceText(line)}，再看相對大盤轉強。`:hot?'先等漲幅／乖離降溫且支撐維持，再評估是否增加部位。':nextEntry;
    result.alternatives=[{title:'續抱',text:!risk&&!weak?'保留部位，持續核對趨勢線、自訂風險線與計畫到期日。':'續抱條件已受損；若保留部位，先寫明可接受的額外風險。'},
      ...
      {title:'退出／重新檢查',text:risk?'自訂風險線已觸發，優先評估退出。實際成交可能有跳空及成本。':`若收盤失守 ${priceText(line)}、觸及成本風險線，或持有理由失效，重新評估退出。`}];
    const nextItems=risk||weak||hot||!positive(nextHigh)||!positive(nextMeanVolume)?[next]:[
      `收盤價：突破 ${priceText(nextHigh)}`,
