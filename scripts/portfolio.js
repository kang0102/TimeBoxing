...
function planningPanel(p,a,demo){
  ...
  const coverage=Research.coverage(fundamentals?.stocks?.find(r=>r.symbol===p.symbol));box.append(node('p','研究材料：'+coverage.label+'。'+coverage.note,'plan-note'));
...
function researchPanel(p,a,plan){
  ...
  function draw(){
    ...
    const calculated=Research.financial(record);if(calculated.length){content.append(node('h4','財報交叉核對：可重算的指標'),node('p',`金額單位：${record.financial?.unit||'依各指標來源'}。各指標另列比較期間與結果單位；公式可重算，解讀屬研究推論。`,'plan-note'));for(const m of calculated){const b=node('article',undefined,'evidence-card');b.append(node('strong',`${m.label}：${num(m.value)}${m.unit||'%'}`),node('p',m.formula,'plan-note'),node('p',m.meaning));if(m.period)b.append(node('small',m.period));if(m.source)b.append(researchLink(m.source));content.append(b);}if(record.financial?.source)content.append(researchLink(record.financial.source));}
